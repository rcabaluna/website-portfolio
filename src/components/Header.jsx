...
const Header = ({ logoColor }) => {
  ...
  const [currentLogoColor, setCurrentLogoColor] = useState(logoColor || "#CCF381");
  const [showLogo, setShowLogo] = useState(true); // track logo visibility
  const dropdownRef = useRef(null);

  // Detect screen size and set logo color
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth <= 640) {
        setCurrentLogoColor("#CCF381");
      } else {
        setCurrentLogoColor(logoColor || "#CCF381");
      }
    };
  ...
  // Fade logo in/out based on scroll position
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY === 0) {
        setShowLogo(true);
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 px-10 py-6 bg-transparent">
      <div className="max-w-screen-xl mx-auto flex items-center relative">
        ...
        {/* Toggle Menu Icon — always stays at right */}
        <button
